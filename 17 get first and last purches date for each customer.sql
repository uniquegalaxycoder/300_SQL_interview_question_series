...
-- q. write a query to get first and last purches date for each customer

select
  a.cust_id,
  a.cust_name,
  min(b.purches_date) as first_purches_date,
  max(b.purches_date) as last_purches_date
from 
  customer as a 
left join 
  sales as b 
...
where 
  b.purches_date is not null
group by 
  a.cust_id,
  a.cust_name
order by min(purches_date) asc;
