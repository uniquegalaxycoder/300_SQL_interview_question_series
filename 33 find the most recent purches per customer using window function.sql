""" 
  Q. find the most recent purches per customer using window function
"""
...
select 
  user_name,
  order_date as latest_purches_date
from (
select 
  *,
  row_number()over(partition by user_id order by order_date desc) as order_no
from cte1 ) as x 
where order_no = 1
